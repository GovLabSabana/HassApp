...
async def list_exportaciones(db: AsyncSession = Depends(get_db)):
    orm_list = await repo.get_all_raw(db)
    salida: List[ExportacionOutWithCosechas] = []
    for exp in orm_list:
        cosechas_read = [CosechaRead.from_orm_with_predios(rel.cosecha) for rel in exp.cosechas]
        salida.append(
            ExportacionOutWithCosechas(
                id=exp.id,
                fecha=exp.fecha,
...
async def create_exportacion(export_in: ExportacionCreate, db: AsyncSession = Depends(get_db)):
    # Crear y luego recargar
    new_raw = await repo.create(db, export_in)
    exp = await repo.get_by_id_raw(db, new_raw.id)
    if not exp:
        raise HTTPException(status_code=500, detail="Error interno al crear exportación")
    cosechas_read = [CosechaRead.from_orm_with_predios(rel.cosecha) for rel in exp.cosechas]
...
async def delete_exportacion(exportacion_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Exportación no encontrada")
    cosechas_read = [CosechaRead.from_orm_with_predios(rel.cosecha) for rel in deleted.cosechas]
    return ExportacionOutWithCosechas(
        id=deleted.id,
        fecha=deleted.fecha,
        metodo_salida=deleted.metodo_salida,
        toneladas=float(deleted.toneladas) if deleted.toneladas is not None else None,
        valor_fob=float(deleted.valor_fob) if deleted.valor_fob is not None else None,

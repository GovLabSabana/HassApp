...
export default function MiExportacion() {
  ...
  const fetchHistoricalData = async () => {
    const token = localStorage.getItem("access_token");
    try {
      const res = await fetch(`${API_URL}/estadisticas/exportaciones/linea-tiempo`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
          ...
          <div className="chart-container" style={{ height: "300px", marginTop: "1rem" }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={historicalData} margin={{ top: 20, right: 30, left: 10, bottom: 5 }}>
                ...
                <ReLine yAxisId="left" type="monotone" dataKey="valor_fob" stroke="#48bb78" strokeWidth={3} dot={{ stroke: "#fff", strokeWidth: 2, fill: "#48bb78", r: 6 }} activeDot={{ r: 8 }} name="Valor FOB" />
                <ReLine yAxisId="right" type="monotone" dataKey="toneladas" stroke="#667eea" strokeWidth={3} dot={{ stroke: "#fff", strokeWidth: 2, fill: "#667eea", r: 6 }} activeDot={{ r: 8 }} name="Toneladas" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>

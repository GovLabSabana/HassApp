...
export default function PropertiesAdd() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    const isNumericField = ['cedula_catastral', 'municipio_id', 'hectareas', 'altitud_promedio'].includes(name);
    ...
    if (isNaN(numberValue) || numberValue < 0) {
        setErrors((prev) => ({ ...prev, [name]: 'Este valor no puede ser negativo' }));
    } else {
        setErrors((prev) => {
  ...
  const validateForm = () => {
    ...
    const numericFields = ['cedula_catastral', 'municipio_id', 'hectareas', 'altitud_promedio'];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const res = await fetch('https://hassapp-production.up.railway.app/predios/', {
        method: 'POST',
        headers: {
      ...
      <main className="properties-main">
        ...
        <form onSubmit={handleSubmit} className="form">
          ...
          <div style={{ marginBottom: '1rem' }}>
            {errors.vocacion && <div style={{ color: 'red', fontSize: '0.9rem' }}>{errors.vocacion}</div>}
          ...
          <div style={{ marginBottom: '1rem' }}>
            {errors.tipo_riego && <div style={{ color: 'red', fontSize: '0.9rem' }}>{errors.tipo_riego}</div>}
